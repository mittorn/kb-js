    ...
    if(req[0] === 'login') {
      ...
      getUserToken(querySender, function(auth_token) {
        if(auth_token) {
          ...
          matrix.sendNotice(queryRoom.roomId, 'It seems you are already logged into Wunderlist. Please use !wunderlist logout if you want to logout and try to log in again.');
        } else {
          ...
          db.run("INSERT INTO auth_requests (id, matrix_user, room_id, secret_key, requested_at) VALUES (?, ?, ?, ?, ?)", auth_request, querySender, queryRoom.roomId, getGUID(), new Date(), function(err) {
            if(err) {
              ...
              matrix.sendNotice(queryRoom.roomId, 'An error occured obtaining the authentication request.');
            } else {
              // Return the URL the user can use to authenticate with Wunderlist...
              matrix.sendNotice(queryRoom.roomId, 'Please authenticate with Wunderlist here: ' + config.myServer + 'login?auth_request=' + encodeURIComponent(auth_request));
            }
          });
      ...
      db.run("DELETE FROM auth_tokens WHERE matrix_user=?", querySender, function(err) {
        if(err) {
          console.log('An error occured with SQLite3: ' + err);
          matrix.sendNotice(queryRoom.roomId, 'An error occured trying to log you out. Please get in touch with my supervisor.');
        } else {
          matrix.sendNotice(queryRoom.roomId, 'You are no longer logged into Wunderlist (we have thrown away your authentification token).');
        }
      });
    } else if(req[0] === 'add') {
      // Add new item
      if(!req[1] || !req[2]) {
        matrix.sendNotice(queryRoom.roomId, 'You have to specify the list to be added to and the text of the item.'); // TODO: support default lists
      } else {
            ...
            request(
              ...
              , function(err, res, body) {
                if(err) {
                  ...
                  matrix.sendNotice(queryRoom.roomId, 'An error occured trying to add your task. Please try again later!');
                } else {
                  matrix.sendNotice(queryRoom.roomId, '[' + listTitle + '] ' + itemToAdd);
                }
      ...
      if(!req[1]) {
        matrix.sendNotice(queryRoom.roomId, 'You have to specify the list you want to follow in this room.');
      } else {
        var listSearch = req[1].match(/^"?(.*?)"?$/)[1];
        findWunderlists(querySender, listSearch, function(err, listId, listTitle, auth_token) {
          if(err) {
            ...
            db.all("SELECT * FROM monitors WHERE matrix_user = ? AND list_id = ?", querySender, listId, function(err, rows) {
              if(rows && rows.length > 0) {
                ...
                if (rows.filter(function(d) { return d['matrix_room'] == queryRoom.roomId; }).length >= 1) {
                  ...
                  matrix.sendNotice(queryRoom.roomId, 'We are already following the list ' + listTitle + '.');
                } else {
                  // Add new room
                  db.run("INSERT INTO monitors (matrix_room, matrix_user, list_id, list_title, webhook_id, secret_key) VALUES (?, ?, ?, ?, ?, ?)", queryRoom.roomId, querySender, listId, listTitle, rows[0]['webhook_id'], rows[0]['secret_key'], function(err) {
                    if(err) {
                      ...
                      matrix.sendNotice(queryRoom.roomId, 'An error occurde while trying to follow ' + listTitle + '. Please try again later.');
                    } else {
                      matrix.sendNotice(queryRoom.roomId, 'We are now following the list ' + listTitle + '.');
                    }
                  });
                }
                ...
                request(
                  ...
                  , function(err, res, body) {
                    if(err) {
                      ...
                      matrix.sendNotice(queryRoom.roomId, 'An error occured trying to obtain notifications from Wunderlist. Please try again later!');
                    } else {
      ...
      if(!req[1]) {
        matrix.sendNotice(queryRoom.roomId, 'You have to specify the list you want to unfollow in this room.');
      } else {
        var listSearch = req[1].match(/^"?(.*?)"?$/)[1];
        findWunderlists(querySender, listSearch, function(err, listId, listTitle, auth_token) {
          if(err) {
            ...
            db.get('SELECT * FROM monitors WHERE matrix_room = ? AND matrix_user = ? AND list_id = ?', queryRoom.roomId, querySender, listId, function(err, row) {
              if(err) {
                ...
                matrix.sendNotice(queryRoom.roomId, 'An error occured trying to obtain existing follows. Please try again later!');
              } else if(!row) {
                matrix.sendNotice(queryRoom.roomId, 'You are not currently following the list ' + listTitle + '.');
              } else {
                        ...
                        request(
                          ...
                          , function(err, res, body) {
                            if(err) {
                              ...
                              matrix.sendNotice(queryRoom.roomId, 'An error occured removing the webhook from Wunderlist. Please try again later!');
                            } else {
                              matrix.sendNotice(queryRoom.roomId, 'We are now no longer following ' + listTitle + '.');
      ...
      matrix.sendNotice(queryRoom.roomId, 'I did not understand this request. Call !help wunderlist for more details.');
    }
  } else {
    matrix.sendNotice(queryRoom.roomId, 'I did not understand this request. Call !help wunderlist for more details.');
  };
      ...
      if(row && row['matrix_user'] && row['secret_key'] === query['state']) {
        ...
        request({
          ...
          if(err || !body || !body['access_token']) {
            ...
            matrix.sendNotice(row['room_id'], 'Sorry, there was a problem obtaining your Wunderlist access token. Please try again!');
          } else {
            db.run('INSERT INTO auth_tokens (matrix_user, auth_token, obtained_at) VALUES (?, ?, ?)', row['matrix_user'], body['access_token'], new Date(), function(err) {
              if(err) {
                ...
                console.log(err);
                matrix.sendNotice(row['room_id'], 'Sorry, there was a problem storing your Wunderlist access token. Please try again!');
              } else {
                matrix.sendNotice(row['room_id'], 'Thank you! Your matrix user is now authenticated for use with Wunderlist.');
              }
            });
          }
        });
      ...
      db.each("SELECT * FROM monitors WHERE list_id=? AND secret_key=?", listId, secretKey, function(err, row) {
        ...
        if(!err && row) {
          ...
          if(res.req.body.operation === 'create') {
            // New task
            matrix.sendNotice(row['matrix_room'], '[' + row['list_title'] + '] New task: ' + res.req.body.after.title);
            ...
            matrix.sendNotice(row['matrix_room'], '[' + row['list_title'] + '] Completed: ' + res.req.body.after.title);
          }
        }
      });
    }
